...
typedef uint16_t t;
t	GreenTaskProfiler, BlueTaskProfiler, RedTaskProfiler, OrangeTaskProfiler;
//int ch[1] = {'H'};
...
int main(void)
{
  ...
  xTaskCreate(vOrangeLedControllerTask,
     		  	  "Orange Led Controller",		//Name string for the task function as our preference
     			   100,							//The stack for the temporary storage that's own current execution state in register bank
     			   NULL,						//the parameters to be passed(arguments) to the TaskFunction here NUll nothing passes
     			   6,							//the priority of the TaskFunction let set it as 1
     			   NULL							//TaskHandler for that task...
     			   );
...
void vGreenLedControllerTask(void* pvParameters)
{
	while(1)
	{
		...
		/*Yielding the task to next queued task to run */
		taskYIELD();
