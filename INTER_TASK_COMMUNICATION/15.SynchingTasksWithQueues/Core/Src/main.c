...
TaskHandle_t sender_handle1,sender_handle2,receiver_handle;
...
int main(void)
{
  ...
  /*TaskCreation: */
   ...
   /*Queue Creation: */
   uC_Name_queue = xQueueCreate(8,sizeof(const char *));
...
void SenderTask1(void *pvParameters)
{
	...
	while(1)
	{
		queue_status = xQueueSend(uC_Name_queue, &Item2Send, portMAX_DELAY);
...
void ReceiverTask(void *pvParameters)
{
	...
	while(1)
	{
		queue_status = xQueueReceive(uC_Name_queue, &ReceiveBuffer,portMAX_DELAY);
