...
int main(void)
{
  ...
  xTaskCreate(vPressureSensorSenderTask,
    ...
    /*Create Queue... */
    qData_Buffer = xQueueCreate(10,sizeof(xStructToSend));
...
void vTempSensorSenderTask(void* pvParameters)
{
	...
	while(1)
	{
		qstatus = xQueueSend(qData_Buffer, &xStructToSend[0],portMAX_DELAY);
...
void vHumiditySensorSenderTask(void* pvParameters)
{
	...
	while(1)
	{
		qstatus = xQueueSend(qData_Buffer, &xStructToSend[1],portMAX_DELAY);
...
void vPressureSensorSenderTask(void* pvParameters)
{
	...
	while(1)
	{
		qstatus = xQueueSend(qData_Buffer, &xStructToSend[2],portMAX_DELAY);
...
void vAllDataReceiverTask(void* pvParameters)
{
	...
	while(1)
	{
		qstatus = xQueueReceive(qData_Buffer, &received_data,portMAX_DELAY);
