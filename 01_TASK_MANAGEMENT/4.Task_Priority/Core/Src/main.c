...
typedef uint16_t t;
t	GreenTaskProfiler, BlueTaskProfiler, RedTaskProfiler;
//int ch[1] = {'H'};
...
int main(void)
{
  ...
  xTaskCreate(vLedControllerTask,
			   ...
			   (void *)blue_led,						//the parameters to be passed(arguments) to the TaskFunction here NUll nothing passes
  ...
  xTaskCreate(vLedControllerTask,
  	  			   ...
  	  			   100,							//The stack for the temporary storage that's own current execution state in register bank
				   (void *)red_led,						//the parameters to be passed(arguments) to the TaskFunction here NUll nothing passes
  	  			   2,							//the priority of the TaskFunction let set it as 1
  	  			   NULL							//TaskHandler for that task...
  	  			   );
  ...
  xTaskCreate(vLedControllerTask,
  	  			   ...
  	  			   100,							//The stack for the temporary storage that's own current execution state in register bank
				   (void *)green_led,						//the parameters to be passed(arguments) to the TaskFunction here NUll nothing passes
  	  			   1,							//the priority of the TaskFunction let set it as 1
  	  			   NULL							//TaskHandler for that task...
  	  			   );
