...
void xAutoreload_callback_task(TimerHandle_t xTimer)
{
	...
	if(timeout_count == 10)
	{
		printf("Auto reload Timer was stopped!!! Current_Tick: %ld \r\n",current_Tick);
		printf("Auto reload Timer was Reseted!!! Current_Tick: %ld \r\n",current_Tick);

		xTimerStop(xAutoreload_handle, 0 );
		//xTimerReset(xAutoreload_handle, 0 );
	}
	else
	{
